...
class UserInfoData {
    ...
    private static let onlineStatus = [true, false]
    ...
    static func generateUser() -> User {
        ...
        let online = onlineStatus[Int(arc4random_uniform(1))]
        ...
        let followers = Int(arc4random_uniform(maxRandomValue))
        var photos = [UIImage]()
        for _ in 0 ..< 4 {
        ...
        return User(name: name, surname: surname, isOnline: online, age: age, city: city, friends: friends, followers: followers, photos: photos, groups: groups, videos: videos, audios: audios, presents: presents, files: files, profileImage: avatar)
    }
}
